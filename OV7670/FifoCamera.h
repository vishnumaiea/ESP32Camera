...
class FifoCamera {
 ...
 public:
  ...
  void prepareCapture() { fifo.writeReset(); }

  void startCapture() { fifo.writeEnable(); }

  void stopCapture() { fifo.writeDisable(); }
  ...
  void inline readFrame(unsigned char *frame, const int xres, const int yres, const int bytes) {
    ...
    for (int y = 0; y < yres; y++)
      for (int x = 0; x < xres; x++)
        for (int b = 0; b < bytes; b++) frame[i++] = fifo.readByte();
  }
