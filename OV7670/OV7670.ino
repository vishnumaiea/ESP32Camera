...
I2C<SIOD, SIOC> i2c;
FifoCamera<I2C<SIOD, SIOC>, RRST, WRST, RCK, WR, D0, D1, D2, D3, D4, D5, D6, D7>
    camera(i2c);
...
void displayRGB565() {
  ...
  for (int x = 0; x < XRES; x++)
    for (int y = 0; y < YRES; y++) {
      ...
      // tft.pushColor(((frame[i] | (frame[i + 1] << 8)) >> 1) &
      // 0b111101111101111); //dimming to test for tft error
    }
}
...
void testTFT() { // a small tft test output showing errors on my tft with bright colors
  ...
  int i = 0;
  for (int y = 0; y < 64; y++)
    for (int x = 0; x < 32; x++) tft.pushColor(x | y << 5);
}
