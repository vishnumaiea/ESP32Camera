...
class Fifo {
 public:
  ...
  static void inline writeEnable() { digitalWrite(WR, 0); }

  static void inline writeDisable() { digitalWrite(WR, 1); }
  ...
  static unsigned char inline readByte() {
    digitalWrite(RCK, 1);
    unsigned char b = digitalRead(D0) | (digitalRead(D1) << 1) |
                      (digitalRead(D2) << 2) | (digitalRead(D3) << 3) |
                      (digitalRead(D4) << 4) | (digitalRead(D5) << 5) |
                      (digitalRead(D6) << 6) | (digitalRead(D7) << 7);
    digitalWrite(RCK, 0);
    return b;
  }
